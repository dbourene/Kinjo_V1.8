...
export class Annexe21Service {
  ...
  static async createOperationACC(
    ...
    // Sanitization des composants pour éviter les caractères spéciaux
    const titulaireSanitized = sanitizeFilename(titulaire);
    const communeSanitized = sanitizeFilename(commune);
    
    // Génération de la dénomination : titulaire_commune
    const denomination = `${titulaireSanitized}_${communeSanitized}`;
  ...
  static async generateAnnexe21File(
    ...
    try {
      ...
      // 3. Génération du nom de fichier avec date actuelle et sanitization
      const currentDate = new Date();
      const dateString = currentDate.toISOString().slice(0, 10).replace(/-/g, ''); // AAAAMMJJ
      const rawFileName = `${operation.denomination}_${dateString}_ANNEXE21.xlsx`;
      const fileName = sanitizeFilename(rawFileName);
      
      console.log('📝 Nom du fichier brut:', rawFileName);
      console.log('📝 Nom du fichier sanitisé:', fileName);
